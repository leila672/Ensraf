        ...
        <div class="card-body">
            <form method="POST" action="{{ route('schools.students.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('academic_level') ? 'is-invalid' : '' }}"
                                ...
                                @foreach (App\Models\Student::ACADEMIC_LEVEL_SELECT as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('academic_level', '') === (string) $key ? 'selected' : '' }}>
                                         {{ trans('global.academic_level.'.$label) }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('class_number') ? 'is-invalid' : '' }}"
                                ...
                                @foreach (App\Models\Student::CLASS_NUMBER_SELECT as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('class_number', '1') === (string) $key ? 'selected' : '' }}>
                                        {{ trans('global.class_number.'.$label) }}</option>
                                @endforeach
                            </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('relative_relation') ? 'is-invalid' : '' }}"
                        name="relative_relation" id="relative_relation" required>
                        <option value disabled {{ old('relative_relation', null) === null ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}</option>
                        @foreach (App\Models\Student::RELATIVE_RELATION_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('relative_relation', '') === (string) $key ? 'selected' : '' }}>
                                {{ trans('global.relative_relation.'.$label) }}</option>
                        @endforeach
                    </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('license_number') ? 'is-invalid' : '' }}"
                                type="number" name="license_number" id="license_number"
                                value="{{ old('license_number', '') }}" step="1">
                ...
                <div class="form-group">
                    <label class="required"
                        for="identity_num">{{ trans('cruds.student.fields.identity_num') }}</label>
                ...
                <div class="form-group">
                    <label class="required"
                        for="identitty_photo">{{ trans('cruds.student.fields.identitty_photo') }}</label>
                    <div class="needsclick dropzone {{ $errors->has('identitty_photo') ? 'is-invalid' : '' }}"
                        id="identitty_photo-dropzone">
                    </div>
